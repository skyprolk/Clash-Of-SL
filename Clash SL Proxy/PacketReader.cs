...
namespace CSP
{
    internal class PacketReader : BinaryReader
    {
        ...
        {
            byte[] homeData = this.ReadBytes();

            if (homeData.Length != 0)
            {
                MemoryStream _MemoryStream = new MemoryStream(homeData);
                using (BinaryReader br = new BinaryReader(_MemoryStream))
                {
                    int decompressedLength = br.ReadInt32();
                    string homeJson = ZlibStream.UncompressString(ReadBytes(decompressedLength));
                    return  homeJson;
                }
            }
            else
